...
// Currently vector output is not supported for OperationNodes
// #define SUPPORT_VECTOR_OUTPUT

// TODO
// Fix up execute_program to have a runtime stack class
// Change std::vector to Vector
...
namespace blender::nodes::node_geo_equation_cc {

// The maximum number of inputs to the node (excluding the expression)
...
static const TokenInfo const token_info[(int)T::NUM] = {
    ...
    {T::ACCESS_FIELD_V, "FN_CONV_I2F", EV::FLOAT, 1, EV::INT, EV::NONE, 7},
};
...
class ExpressionParser {
 public:
  ...
  std::vector<short> *input_types_;
  const char *error_msg_ = "";
  int error_pos_ = -1;

  ExpressionParser(std::vector<const char *> *input_names, std::vector<short> *input_types)
...
class ExpressionProgram {
 ...
 public:
  ExpressionProgram(std::vector<const char *> &input_names,
                    std::vector<short> input_types,
                                    ...
                                    eValueType &arg2_type)
  {
    ...
    if (convert_op != TokenType::NONE) {
      ...
      if (specialized_op != TokenType::NONE) {
        output.add_token(convert_op, 1);  // Convert first arg (1 above stack top)
                              ...
                              std::string &error_msg)
  {
    ...
    if (top_type == eValueType::VEC && output_type_ != eNodeSocketDatatype::SOCK_VECTOR) {
      ...
      output.add_token(TokenType::ACCESS_FIELD_V, 2);
    }
  ...
  output_variant execute_program(GVArray inputs[], int index) const
  {
    if (!program_valid_)
      return 0;
    ...
    for (int i = 0; i < num_ops; i++) {
      ...
      switch (t.type) {
        ...
        case Token::TokenType::ACCESS_FIELD_V: {
          int offset = t.value;
          float f = stack[top_idx - offset];
...
class EquationEvaluateFunction : public mf::MultiFunction {
  ...
  std::vector<std::string> input_identifiers_;
  std::vector<short> input_types_;
  const bNode &node_;
  std::unique_ptr<ExpressionProgram> program_;

 public:
  EquationEvaluateFunction(const bNode &node, std::unique_ptr<ExpressionProgram> program)
      : node_(node), program_(std::move(program))
  {
    const NodeGeometryEquation *node_storage = static_cast<NodeGeometryEquation *>(node.storage);
    ...
    // Build a vector of input socket names and identifiers (excluding equation input)
    input_names_.clear();
    for (int i = 1; i < node.input_sockets().size(); i++) {
      auto in_sock = node.input_sockets()[i];
      if (in_sock->typeinfo->base_cpp_type != nullptr) {
        input_names_.push_back(in_sock->name);
        input_identifiers_.push_back(in_sock->identifier);
        input_types_.push_back(in_sock->type);
      }
    }
  }

  // virtual ~EquationEvaluateFunction() {}

  void CreateSignature(const bNode &node)
  {
    mf::SignatureBuilder builder{"Equation", signature_};

    // Create the input parameters, skipping unconnected extend socket
    for (int i : node.input_sockets().index_range()) {
      if (i == 0)  // Skip equation input
        continue;
      auto in_sock = node.input_sockets()[i];
      if (in_sock->typeinfo->base_cpp_type != nullptr)
    ...
    if (program_->output_type_ == eNodeSocketDatatype::SOCK_FLOAT ||
        program_->output_type_ == eNodeSocketDatatype::SOCK_INT ||
        program_->output_type_ == eNodeSocketDatatype::SOCK_BOOLEAN)
    {
  ...
  void call(const IndexMask &mask, mf::Params params, mf::Context context) const final
  {
    ...
    GVArray input_arrays[MAX_INPUTS];
    for (int i = 0; i < input_names_.size(); i++) {
      input_arrays[i] = params.readonly_single_input(i, input_identifiers_.at(i));
    }
...
// #define TEST
      ...
      mask.foreach_index([&](const int64_t i) {
        float sum = 0;
        for (int n = 0; n < input_names_.size(); n++) {
          float val;
      ...
      // Vector output type
      GVectorArray_TypedMutableRef v_results = params.vector_output<float3>(0, "Result");
      mask.foreach_index([&](const int64_t i) {
        auto val = program_->execute_program(input_arrays, i);
...
NODE_STORAGE_FUNCS(NodeGeometryEquation);
...
#else
  return ELEM(data_type, SOCK_FLOAT, SOCK_INT, SOCK_BOOLEAN);
#endif
}
...
static void node_declare(NodeDeclarationBuilder &b)
{
  ...
  // b.add_input<decl::Float>("X").default_value(1.0f).supports_field();
  // b.add_input<decl::Float>("Y").default_value(1.0f).supports_field();
  // b.add_input<decl::Float>("Z").default_value(1.0f).supports_field();

  // Add the variable number of input sockets
  const NodeGeometryEquation &storage = node_storage(*node);
  for (const NodeEquationItem &eq_item : storage.socket_items.items()) {
    const std::string identifier = EquationItemsAccessor::socket_identifier_for_item(eq_item);
    ...
    auto &input = b.add_input(dataType, eq_item.name, identifier)
                      .socket_name_ptr(
                          &ntree->id, EquationItemsAccessor::item_srna, &eq_item, "name");
    input.supports_field();
  }

  // Add extension socket if possibe
  // NOTE not displaying the extension causes a crash
  //  if (storage.socket_items.items_num < MAX_INPUTS)
...
{
  NodeGeometryEquation *data = MEM_cnew<NodeGeometryEquation>(__func__);
  ...
  data->socket_items.items_array = MEM_cnew_array<NodeEquationItem>(2, __func__);
  EquationItemsAccessor::init_with_socket_type_and_name(
...
{
  NodeGeometryEquation *data = reinterpret_cast<NodeGeometryEquation *>(node->storage);
  if (!data)
    return;

  socket_items::destruct_array<EquationItemsAccessor>(*node);
...
{
  const NodeGeometryEquation &src_storage = node_storage(*src_node);
  NodeGeometryEquation *dst_storage = (NodeGeometryEquation *)MEM_cnew<NodeGeometryEquation>(
      __func__, src_storage);
  dst_node->storage = dst_storage;

  socket_items::copy_array<EquationItemsAccessor>(*src_node, *dst_node);
}

static bool node_insert_link(bNodeTree *ntree, bNode *node, bNodeLink *link)
{
  auto storage = reinterpret_cast<NodeGeometryEquation *>(node->storage);
  int starting_sockets_num = storage->socket_items.items_num;
  if (starting_sockets_num >= MAX_INPUTS)
    return false;

  bool ok = socket_items::try_add_item_via_any_extend_socket<EquationItemsAccessor>(
  ...
  if (starting_sockets_num == storage->socket_items.items_num) {
    if (!EquationItemsAccessor::supports_socket_type((eNodeSocketDatatype)(link->fromsock->type)))
      return false;
  }
  ...
  if (link->fromnode->is_group_input() || link->fromnode->is_group_output())
    return ok;
  auto from_type = link->fromnode->type;
  if (from_type == GEO_NODE_REPEAT_INPUT || from_type == GEO_NODE_REPEAT_OUTPUT ||
      from_type == GEO_NODE_FOREACH_GEOMETRY_ELEMENT_INPUT ||
      from_type == GEO_NODE_FOREACH_GEOMETRY_ELEMENT_OUTPUT || from_type == NODE_GROUP)
    return ok;
  ...
  if (item_index == 0)
    ...
    // If the previous name was a single letter, set the new name to the next in the alphabet
    auto prev_name = storage->socket_items.items_array[item_index - 1].name;
    if (strlen(prev_name) == 1) {
      char first_char = prev_name[0];
      char next_char = first_char == 'z'                     ? 'a' :
                       first_char == 'Z'                     ? 'A' :
                       first_char >= 'a' && first_char < 'z' ? first_char + 1 :
                       first_char >= 'A' && first_char < 'Z' ? first_char + 1 :
                                                               0;
      if (next_char) {
        const char fc[2]{next_char, 0};
        new_name = fc;

        // Check we're not making a duplicate
        for (int i = 0; i < storage->socket_items.items_num - 1; i++) {
          if (strcmp(storage->socket_items.items_array[i].name, new_name) == 0) {
            new_name = nullptr;
            break;
          }
        }
...
static void node_geo_exec(GeoNodeExecParams params)
{
  ...
  // Get the equation
  std::string equation = params.get_input<std::string>("Equation");

  // If no equation, do nothing
  if (equation.size() == 0) {
    // params.error_message_add(NodeWarningType::Error, TIP_("Equation"));
  ...
  const NodeGeometryEquation &storage = *(const NodeGeometryEquation *)(node.storage);
  uint8_t oType = storage.output_type;
  // const CPPType &output_type = (oType == SOCK_FLOAT ? CPPType::get<float>() :
  //                               oType == SOCK_INT   ? CPPType::get<int32_t>() :
  //                                                     CPPType::get<float3>());

  // Build vectors of input names and types (excluding equation socket, and extend socket)
  std::vector<const char *> input_names;
  std::vector<short> input_types;
  for (int i = 1; i < node.input_sockets().size(); i++) {
    auto in_sock = node.input_sockets()[i];
    if (in_sock->typeinfo->base_cpp_type != nullptr) {
      input_names.push_back(in_sock->name);
      input_types.push_back(in_sock->type);
    }
  }
  ...
  if (!program->create_program(equation, error_msg)) {
  ...
  auto mf = new EquationEvaluateFunction(params.node(), std::move(program));
  GField f_calculated_results{FieldOperation::Create(*mf, input_fields)};
...
static void node_register()
{
  ...
  geo_node_type_base(&ntype, GEO_NODE_EQUATION, "Evaluate an equation", NODE_CLASS_CONVERTER);
  ...
  blender::bke::node_type_storage(
      &ntype, "NodeGeometryEquation", node_free_storage, node_copy_storage);

  node_rna(ntype.rna_ext.srna);

  //  node_fn_equation_cc::node_rna(ntype.rna_ext.srna);
}
...
StructRNA *EquationItemsAccessor::item_srna = &RNA_NodeEquationItem;
int EquationItemsAccessor::node_type = GEO_NODE_EQUATION;
int EquationItemsAccessor::item_dna_type = SDNA_TYPE_FROM_STRUCT(NodeEquationItem);

void EquationItemsAccessor::blend_write_item(BlendWriter *writer, const ItemT &item)
{
  BLO_write_string(writer, item.name);
}

void EquationItemsAccessor::blend_read_data_item(BlendDataReader *reader, ItemT &item)
{
...
blender::Span<NodeEquationItem> NodeEquationItems::items() const
{
  return blender::Span<NodeEquationItem>(items_array, items_num);
}

blender::MutableSpan<NodeEquationItem> NodeEquationItems::items()
{
  return blender::MutableSpan<NodeEquationItem>(items_array, items_num);
}
